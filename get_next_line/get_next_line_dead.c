...
/*   Updated: 2021/11/26 11:25:39 by nvideira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_update_storage(char **string, char **storage)
{
	unsigned int	i;
	char			*vivi;
...
char	*read_to_buffer(int fd, char *string, char **storage)
{
	int		rd_check;
	char	*buffer;
...
char	*get_next_line(int fd)
{
	...
	if (storage)
	{
		add_update_storage(&string, &storage);
		if (ft_strchr(string, '\n'))
			return (string);
	}
	if (!ft_strchr(storage, '\n'))
		string = read_to_buffer(fd, string, &storage);
	return (string);
}
